...
# run on the original MoneyPuck data set.
...
# This one is actually an analysis choice. I'm not interested in shots taken from outside the blue line.
df_Nzone = df[df['x'] < 25]
...
df_oc_py.drop( df_oc_py[oc_py_r.values < corner_rad-0.5].index,
inplace=True)
...
df_oc_ny.drop( df_oc_ny[oc_ny_r.values < corner_rad-0.5].index,
inplace=True)
